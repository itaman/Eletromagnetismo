...
def dados ():
	...
	#while l<=0:
	 #   l=input('\nEntre com a altura do capacitor: ')
	d=input('\nEm quantas divisoes o capacitor eh particionado ao longo de sua circuferencia em materiais diferentes acompanhando o angulo azimutal?\n')
	print '\nPara cada divisao, entre com o numero de camadas de dieletricos diferentes:\n'
	global camadas
	camadas = [0]*d
	for k in range(d):
	    camadas[k]=input('Divisao %d - Numeros de camadas (cascas): ' %(k+1))
...
def calcC():
	...
	Cs=0
	for j in range(camadas[k]):
	    Cs = Cs + (1/Rinf[k,j]-1/Rsup[k,j])/(2*ang[k]*epsilon0*epsilonRi[k,j])
