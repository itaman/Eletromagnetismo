...
def dados ():
	...
	muda=raw_input('Desesja mudar os dados? (S ou N): ')
    if muda in ['S', 's']:
	d=input('\nEm quantas divisões o capacitor é particionado ao longo de sua circuferência em materiais diferentes acompanhando o ângulo azimutal?\n')
	print '\nPara cada divisão, entre com o número de camadas de dielétricos diferentes:\n'
	global camadas
	camadas = [0]*d
	for k in range(d):
	    camadas[k]=input('Divisão %d - Números de camadas (cascas): ' %(k+1))
